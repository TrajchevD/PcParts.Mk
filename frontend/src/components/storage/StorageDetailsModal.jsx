...
import { fetchStorageDetails } from "../../api/storageProducts";

export default function StorageDetailsModal({ productId, onClose, onAdd }) {
  const [data, setData] = useState(null);
  const modalRef = useRef(null);

  useEffect(() => {
    if (!productId) return;
    fetchStorageDetails(productId).then(setData);
  }, [productId]);
            ...
            <div className="details-left">
              ...
              <div className="details-specs">
                ...
                <table className="spec-table">
                  <tbody>
                    <tr><td>Type</td><td>{data.specs.storage_type}</td></tr>
                    <tr><td>Capacity</td><td>{data.specs.capacity_gb} GB</td></tr>
                    <tr><td>Interface</td><td>{data.specs.interface}</td></tr>
                    <tr><td>Form factor</td><td>{data.specs.form_factor}</td></tr>
                    <tr><td>Protocol</td><td>{data.specs.protocol}</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
